...
var FireCrawlClient = class {
  ...
  async scrapeMultiple(urls, options) {
    ...
    const invalidUrls = validations.map((v, i) => ({ validation: v, url: urls[i] })).filter(({ validation }) => !validation.valid);
...
function createFireCrawlClient(env) {
...
async function createConversation(userId, organizationId, context, env) {
...
async function getConversation(conversationId, env) {
  const supabase = getSupabaseClient(env);
...
async function getRecentMessages(conversationId, limit = 10, env) {
  const supabase = getSupabaseClient(env);
...
async function saveUserMessage(conversationId, content, env) {
  const supabase = getSupabaseClient(env);
...
async function saveAssistantMessage(conversationId, content, model, tokensInput, tokensOutput, costUsd, env) {
  const supabase = getSupabaseClient(env);
  const { data, error } = await supabase.from("messages").insert({
    conversation_id: conversationId,
    role: "assistant",
...
async function updateConversationTitle(conversationId, title, env) {
  const supabase = getSupabaseClient(env);
...
async function callClaude(request, env, maxRetries = 3) {
...
async function callClaudeStream(request, env) {
  const apiKey = env.CLAUDE_API_KEY;
  if (!apiKey) {
    throw new Error("CLAUDE_API_KEY not configured");
  }
